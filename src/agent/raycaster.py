...
if __name__ == "__main__":
    ...
    walls = [
        ...
        pygame.Rect(300, 100, 20, 300),  # Obstacle
    ]
    
    raycaster = SimpleRaycaster(max_range=200.0)
    ...
    while running:
        ...
        if keys[pygame.K_RIGHT]:
            agent_dir += 0.05
        if keys[pygame.K_UP]:
            agent_x += math.cos(agent_dir) * 3
            agent_y += math.sin(agent_dir) * 3
        ...
        # Rays
        raycaster.draw(screen, agent_x, agent_y, agent_dir, walls)
        
        # Agent
        pygame.draw.circle(screen, (0, 0, 255), (int(agent_x), int(agent_y)), 10)
        
        # Direction indicator
        dx = math.cos(agent_dir) * 20
        dy = math.sin(agent_dir) * 20
        pygame.draw.line(screen, (255, 255, 255), 
                        (int(agent_x), int(agent_y)),
                        (int(agent_x + dx), int(agent_y + dy)), 2)
        
        # Show distances
        distances = raycaster.cast_rays(agent_x, agent_y, agent_dir, walls)
        labels = ['L', 'F', 'R']
        for i, d in enumerate(distances):
            text = pygame.font.Font(None, 24).render(f"{labels[i]}: {d:.0f}", True, (0, 0, 0))
